...
В каталоге `hooks/` лежат серверные git-хуки:

| Хук         | Что делает                                          |
|-------------|-----------------------------------------------------|
| `pre-commit` | `./mvnw -q compile` — коммит запрещён при падении компиляции |
| `pre-push`   | `./mvnw -q clean test` — push запрещён при падении тестов  |

Хуки версионируются в репозитории и подключаются один раз на локальной машине:

```bash
git config core.hooksPath hooks
```

После этого хуки подхватятся автоматически. Отключить локально: `git config --unset core.hooksPath`.
Для единичного пропуска: `git commit --no-verify` / `git push --no-verify`.
